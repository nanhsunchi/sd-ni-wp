...
% save figure
saveas(gcf,append('spectra_uv_',year,'-SD',platf_num,'_',...
...
% save figure
saveas(gcf,append('time-depth_uv_detide_',year,'-SD',platf_num,'_',...
    string(datetime(tlim_plot(1),'format','yMMd')),'_', ...
    string(datetime(tlim_plot(2),'format','MMd')),'.png'))
...
for i = 1:numel(varnms_plot)
    ...
    %%%
    nt = numel(data);
    nbin = 32; % no of frequency bands in request.
    bins = newcrtbins( nt,2,2,nbin );
    dof = 2*( diff(bins')+1 );
...
% save figure
saveas(gcf,append('spectra(nbin=',num2str(nbin),')_uv-detide_',year,'-SD',platf_num,'_',...
    string(datetime(tlim_plot(1),'format','yMMd')),'_', ...
    string(datetime(tlim_plot(2),'format','MMd')),'.png'))

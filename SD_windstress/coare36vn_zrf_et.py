...
if __name__ == '__main__':
    ...
    path = '/Users/chi/Documents/nhchi_work_2023/2019JulyCase/COARE_cal/'
    
    year = '2019'
    SD_name = '1036'
    timelim = [datetime.datetime(int(year),6,15),datetime.datetime(int(year),7,8,15)]
    pathSD = os.path.expanduser('~/Documents/Data/Arctic'+year+'/')
    filenames = np.sort( os.listdir(pathSD) )
    str_notwant = ['SBE','adcp']
    for f in range(len(filenames)):
    # check if this is the data we want to plot
        if os.path.isfile(pathSD+filenames[f]) & np.invert(any(substring in filenames[f] for substring in str_notwant)) & \
            (SD_name in filenames[f]):
            print(filenames[f])
            ds = nc.Dataset(pathSD+filenames[f])
            time = ds.variables['time'] # nc class variable
            ...
            uwnd = np.squeeze(ds.variables['UWND_MEAN'][:]) # m/s. Installed height 5.2 m. 60s on, 240s off, centered at :00
            vwnd = np.squeeze(ds.variables['VWND_MEAN'][:])
            break
        
    _, jd = datetime_to_yearday(dtime_arr)
    u = np.sqrt(np.square(uwnd) + np.square(vwnd))
    zu= 5.2*np.ones(len(u),)
    t = np.squeeze(ds.variables['TEMP_AIR_MEAN'][:])
    zt= 2.3*np.ones(len(u),)
    rh = np.squeeze(ds.variables['RH_MEAN'][:])
    zq= 2.3*np.ones(len(u),)
    P = np.squeeze(ds.variables['BARO_PRES_MEAN'][:])
    ts = np.squeeze(ds.variables['TEMP_SBE37_MEAN'][:])
    Ts_depth = -0.5*np.ones(len(u),)
    Ss = np.squeeze(ds.variables['SAL_SBE37_MEAN'][:])
    sw_dn = np.squeeze(ds.variables['PAR_AIR_MEAN'][:])/(2.4) # convert micromol/s/m2 to w/m2 for summer Pacific Arctic
    lw_dn = 350*np.ones(len(u),)
    lat = np.squeeze(ds.variables['latitude'][:])
    lon = np.squeeze(ds.variables['longitude'][:])
    zi = 600*np.ones(len(u),) # defailt or typical value = 600 m
    rain = np.zeros(len(u),) ### No rainrate data. Assume No Rain here
    zrf_u = 10.0*np.ones(len(u),)
    zrf_t = 10.0*np.ones(len(u),)
    zrf_q = 10.0*np.ones(len(u),)
    sigH = np.squeeze(ds.variables['WAVE_SIGNIFICANT_HEIGHT'][:]) # significant wave height (m)
    cp = np.divide(sigH,np.squeeze(ds.variables['WAVE_DOMINANT_PERIOD'][:])) # phase speed of dominant waves (m/s) computed from peak period
    
    # select a period of time to compute
    it = (dtime_arr>= timelim[0]) & (dtime_arr<= timelim[1]) & (np.invert(np.isnan(u))) & \
        (np.invert(np.isnan(t))) & (np.invert(np.isnan(rh))) & (np.invert(np.isnan(P))) & \
            (np.invert(np.isnan(ts))) & (np.invert(np.isnan(Ss))) & (np.invert(np.isnan(sw_dn))) & \
                (np.invert(np.isnan(cp)))
    
    A = coare36vn_zrf_et(u[it], zu[it] , t[it], zt[it], rh[it], zq[it], P[it], ts[it], sw_dn[it], lw_dn[it], lat[it], lon[it], jd[it],\
                          zi[it],rain[it], Ss[it], cp[it], sigH[it], zrf_u[it], zrf_t[it], zrf_q[it])
    fnameA = os.path.join(path,'test_36_output_'+year+'SD'+SD_name+'_withwavesinput.txt')
